...
def get_user_representation(user):
    ...
    if isinstance(show_user_as, str):
        ...
        if attr:
            return str(attr)
    elif isinstance(show_user_as, collections.Callable):
        try:
...
class Message(models.Model):
    ...
    def get_admin_url(self):
        return reverse('postman_admin_view', args=[self.pk])
    ...
    def is_deleted(self):
        ...
        return self.recipient_deleted_at is not None or self.sender_deleted_at is not None

    def viewable(self, user):
        """Returns true if the user can view the message"""
        return (
            self.sender == user or
            self.recipient == user
            )

    def admin_viewable(self, user):
        """Returns true if the user can view the message as an administrator"""
        if self.sender.username=="SYSTEM":
            return user.profile.is_admin(self.recipient.profile.sharezone)
        else:
            return user.profile.is_admin(self.sender.profile.sharezone) and user.profile.is_admin(self.recipient.profile.sharezone)
        # return (
        #     (self.sender.profile is None or user.profile.is_admin(self.sender.profile.sharezone)) and
        #     user.profile.is_admin(self.recipient.profile.sharezone)
        #     )
