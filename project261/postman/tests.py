...
class BaseTest(TestCase):
    ...
    def reload_modules(self):
        ...
        clear_url_caches()
        try:
            imp.reload(sys.modules['postman.utils'])
            imp.reload(sys.modules['postman.fields'])
            imp.reload(sys.modules['postman.forms'])
            imp.reload(sys.modules['postman.views'])
            imp.reload(sys.modules['postman.urls'])
        except KeyError:  # happens once at the setUp
            pass
        imp.reload(get_resolver(get_urlconf()).urlconf_module)
