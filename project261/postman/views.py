...
def _get_referer(request):
    ...
    if 'HTTP_REFERER' in request.META:
        sr = urllib.parse.urlsplit(request.META['HTTP_REFERER'])
        return urllib.parse.urlunsplit(('', '', sr.path, sr.query, sr.fragment))
...
class AdminInboxView(FolderMixin, TemplateView):
    """
    Creates a view for the Admin to be able to see all messages
    across the system (SuperUser), or just the sharezone (sharezone admin). 
    He can also delete or archive messages as he sees fit.
    """
    
    
    #Overwriting the Mixin
    def get_context_data(self, **kwargs):
        # if you are not a superuser or a sharezone admin for this sharezone you cannot access this
        if not (self.request.user.profile.is_admin(self.request.user.profile.sharezone)):
            raise Http404
        context = super(FolderMixin, self).get_context_data(**kwargs)
        params = {}
        option = kwargs.get('option')
        ...
        if order_by:
            params['order_by'] = order_by
        #Superuser can view ALL the messages!
        if(self.request.user.profile.is_admin()):
            msgs = Message.objects.all()
        #admin can just view sharezone system ones
        else:
            msgs = Message.objects.filter(
                sender__profile__sharezone = self.request.user.profile.sharezone
                )
    ...
    folder_name = 'admin inbox'
    view_name = 'postman_admin_inbox'
    # for TemplateView:
    template_name = 'postman/admin_inbox.html'
...
class DisplayMixin(object):
    ...
    def get(self, request, *args, **kwargs):
        user = request.user
        self.msgs = Message.objects.filter(pk=self.message_id)
        if not self.msgs:
            raise Http404
        for msg in self.msgs:
            if not (msg.viewable(user) or (self.admin and msg.admin_viewable(user))):
                raise Http404
...
class AdminMessageView(MessageView):
    """Display one specific message to the admin."""
    admin = True
    template_name = 'postman/admin_view.html'
...
class UpdateMessageMixin(object):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if pks or tpks:
            ...
            if not self.admin: #Page reached from normal inbox
            ...
            else: #Page reached from admin inbox
                #Handle admin modification here
                msgs = Message.objects.filter(pk__in=pks)
                for m in msgs:
                    if not m.admin_viewable(user):
                        raise Http404 #You don't admin these messages, stahp
                msgs.update(**{'sender_{0}'.format(self.field_bit): self.field_value})
                msgs.update(**{'recipient_{0}'.format(self.field_bit): self.field_value})
...
class AdminArchiveView(ArchiveView):
    """Archive messages as the admin"""
    admin=True

class AdminDeleteView(DeleteView):
    """Delete messages as the admin"""
    admin=True

class AdminUndeleteView(UndeleteView):
    """Undelete messages as the admin"""
    admin=True