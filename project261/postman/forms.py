...
class BaseWriteForm(forms.ModelForm):
    ...
    error_messages = {
        ...
        'diff_sharezone': _("You can only message people within your sharezone."),
    }
    def clean_recipients(self):
        ...
        for recipient in recipients:
            print("Recipient Sharezone: "+ recipient.profile.sharezone.name+" and Sender Sharezone: "+sender.profile.sharezone.name)
            #NO messaging people in other sharezones, unless you're the superduper admin or messaging them
            if((recipient.profile.sharezone != sender.profile.sharezone)):
                print("Hit this first if")
                if(not sender.profile.is_admin()):
                    print("Hit second if")
                    if(not recipient.profile.is_admin()):
                        print("Hit third if")
                        errors.append(self.error_messages['filtered_user_with_reason'].format(username= recipient.username, reason = "This user is not in your sharezone."))    
        if errors:
